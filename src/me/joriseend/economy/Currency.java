...
public class Currency extends JavaPlugin {
	...
	String ActualMoney = EconomyPrefix + ChatColor.RED + "You cannot send negative money";
	...
	String YouNowHaveDifferentMoney = EconomyPrefix + ChatColor.GRAY + "Your balance is now" + " " + ChatColor.RESET;
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
		if(command.getName().equalsIgnoreCase("pay")){
			if(sender instanceof Player){
				...
				if(args.length == 0){
					...
					if(Bukkit.getPlayer(args[0]) == null){
						...
						if(amount <= 0){
							player.sendMessage(ActualMoney);
							return true;
						}else{
				...
				if(player.hasPermission("economy.manage.addbalance")){
					if(args.length == 0){
						...
						if(Bukkit.getPlayer(args[0]) == null){
							...
							saveConfig();
							target.sendMessage(YouGotMoney + ChatColor.GOLD + amount);
							player.sendMessage(EconomyPrefix + "Successfully added" + " " + ChatColor.GOLD + amount + ChatColor.RESET + " " + "to" + " " + ChatColor.GOLD + target.getDisplayName() + ChatColor.RESET + "'s" + " " + "balance");
							return true;
						}
					}else if(args.length > 2){
						player.sendMessage(TooManyArgs);
				...
				if(player.hasPermission("economy.manage.rembalance")){
					if(args.length == 0){
						...
						if(Bukkit.getPlayer(args[0]) == null){
							...
							player.sendMessage(EconomyPrefix + "Successfull deducted" + " " + ChatColor.GOLD + amount + ChatColor.RESET + " " + "from" + " " + ChatColor.GOLD + target.getDisplayName() + ChatColor.RESET + "'s" + " " + "balance");
							return true;
						}
					}else if(args.length > 2){
						player.sendMessage(TooManyArgs);
			...
			if(sender instanceof Player){
				Player player = (Player) sender;
				if(player.hasPermission("economy.manage.setbalance")){
					if(args.length == 0){
						player.sendMessage(EconomyPrefix + ChatColor.RED + FewArgsPlayer);
						return true;
					}else if(args.length == 1){
						...
						if(Bukkit.getPlayer(args[0]) == null){
							...
							Player target = Bukkit.getPlayer(args[0]);
							int amount = Integer.parseInt(args[1]);
							getConfig().set(target.getUniqueId().toString() + ConfigBalance, amount);
							saveConfig();
							target.sendMessage(YouNowHaveDifferentMoney + ChatColor.GOLD + amount);
							player.sendMessage(EconomyPrefix + "Successfully set the balance of" + " " + ChatColor.GOLD + target.getDisplayName() + ChatColor.RESET + " " + "to" + " " + ChatColor.GOLD + amount);
							return true;
						}
					}else if(args.length > 2){
						player.sendMessage(TooManyArgs);
